...
public class CreateEncodingWithSprites
{
    ...
    public void testEncoding() throws IOException, BitmovinApiException, UnirestException, URISyntaxException, RestException, InterruptedException
    {
        ...
        encoding.setName("Encoding with sprites");
        ...
        for (MultiStreamRepresentation representation : representations)
        {
            ...
            spriteStream.setCodecConfigId(representation.getConfiguration().getId());
            spriteStream.setInputStreams(Collections.singleton(inputStreamVideo));

            spriteStream = bitmovinApi.encoding.stream.addStream(encoding, spriteStream);
            bitmovinApi.encoding.sprite.create(encoding, spriteStream, new Sprite(
                    240,
                    320,
                    1.0D,
                    "mySprite_%number%.jpeg",
                    "mySprite.vtt",
                    Collections.singleton(spriteOutput),
                    25
            ));

            representation.addStream(
                    spriteStream
            );
        }
