...
import com.bitmovin.api.encoding.encodings.streams.StreamMetadata;
...
/**
 * This example demonstrates how to assign arbitrary labels to an Encoding so that you may breakdown encoding metrics by
 * label when querying the Statistics endpoint:
 *
 * https://bitmovin.com/docs/encoding/api-reference/sections/statistics#/Encoding/GetEncodingStatisticsLabelsDaily
 */
public class CreateMp4EncodingWithLabels
{
    ...
    public void testEncoding() throws IOException, BitmovinApiException, UnirestException, URISyntaxException, RestException, InterruptedException
    {
        ...
        encoding.setName("Encoding Java Example - Add Labels to Encoding");
        encoding.setCloudRegion(CLOUD_REGION);

        // Relevant portion to assign labels to encoding
        List<String> labels = new ArrayList<>();
        labels.add("Customer 1");
        encoding.setLabels(labels);
    ...
    private void createMP4Muxing(Encoding encoding, Output output, Stream videoStream, Stream audioStream, String filename) throws BitmovinApiException, IOException, RestException, URISyntaxException, UnirestException
    {
