...
/**
 * Created by Germano Fronza on 11.21.17.
 */
public class CreateEncodingWithWatermarkFilter
{
    private static String ApiKey = "8ce255f9-c0c6-4401-b6a2-e89661d5fddd";

    private static CloudRegion cloudRegion = CloudRegion.AWS_US_EAST_1;
    private static String HTTPS_INPUT_HOST = "https://s3.amazonaws.com/"; // ex.: storage.googleapis.com/
    private static String HTTPS_INPUT_PATH = "germano-videos/inputs/1080p.mp4";
    private static String S3_OUTPUT_ACCESSKEY = "AKIAJUJPU4CA5A6CQQMQ";
    private static String S3_OUTPUT_SECRET_KEY = "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";
    private static String S3_OUTPUT_BUCKET_NAME = "germano-videos";
    private static String OUTPUT_BASE_PATH = "outputs/watermark/" + new Date().getTime();
    ...
    public void testEncoding() throws IOException, BitmovinApiException, UnirestException, URISyntaxException, RestException, InterruptedException
    {
        ...
        watermarkFilter.setImage("http://via.placeholder.com/75x75");
        watermarkFilter = bitmovinApi.filter.watermark.create(watermarkFilter);
        
        // create the object to connect the stream to the filter
        StreamFilter streamFilter = new StreamFilter();
        streamFilter.setFilter(watermarkFilter);
        streamFilter.setId(watermarkFilter.getId());
        streamFilter.setPosition(0);
        
        // create the filter list and add the watermark filter to it
        StreamFilterList streamFilterList = new StreamFilterList();
        streamFilterList.getFilters().add(streamFilter);
        
        // for each video stream add the filter list
