...
public class CreateMultiCodecEncodingWithDashHls
{
			...
			URISyntaxException, RestException, InterruptedException, InvalidCodecException
	{
		...
		// create videoConfig, stream and muxings for each of the video encoding profies.
		...
		// create audioConfig, stream and muxings for each of the audio encoding profies.
		...
		for (ManifestType manifestType : MANIFESTS)
		{
			switch (manifestType)
			{
			case DASH: manifestsToStart.add(createDashManifest(encoding, output)); break;
			case HLS: manifestsToStart.add(createHlsManifest(encoding, output)); break;
			}
		}
		...
		for (Manifest manifest : manifestsToStart)
		{
			...
			System.out.println(String.format("%s generation finished with status %s", manifest.getClass().getName().toString(), manifestStatus.toString()));
        ...
        for (AACAudioProfile audioProfile : AUDIO_ENCODING_PROFILES)
		{
			for (Muxing muxing : audioProfile.muxings)
			{
				if (muxing.getType() == MuxingType.TS)
				{
			        ...
			        audioMediaInfo.setUri(String.format("audio_%s_%d.m3u8", audioProfile.getCodecType().toString().toLowerCase(), audioProfile.bitrate));
            ...
            for (VideoProfile videoProfile : reverseList(VIDEO_ENCODING_PROFILES))
	    		{
	    			...
	    			{
	    				if (muxing.getType() == MuxingType.TS || (muxing.getType() == MuxingType.FMP4 && videoProfile.getCodecType() == ConfigType.H265))
	    				{
	    					String path = muxing.getOutputs().get(0).getOutputPath().replaceAll(OUTPUT_BASE_PATH, "");

	    					this.addStreamInfoToHlsManifest(String.format("video_%s_%dp_%d.m3u8", videoProfile.getCodecType().toString().toLowerCase(),
	    					        videoProfile.height, videoProfile.bitrate), encoding.getId(), videoProfile.stream.getId(), muxing.getId(),
	    					        audioGroupId, path, hlsManifest);
	    				}
	    			}
	    		}
		}
				...
				if (videoProfile.getCodecType() == codec)
				{
					for (Muxing muxing : videoProfile.muxings)
					{
						if (muxing.getType() == MuxingType.FMP4 || muxing.getType() == MuxingType.WEBM)
						{
							...
							this.addDashRepresentationToAdaptationSet(DashMuxingType.TEMPLATE, encoding.getId(), videoProfile.stream.getId(),
								muxing, path, dashManifest, period, videoAdaptationSet);
						}
					}
				}
			}
		...
		for (AACAudioProfile audioProfile : AUDIO_ENCODING_PROFILES)
		{
			...
			for (Muxing muxing : audioProfile.muxings)
			{
				if (muxing.getType() == MuxingType.FMP4)
				{
					...
					this.addDashRepresentationToAdaptationSet(DashMuxingType.TEMPLATE, encoding.getId(), audioProfile.stream.getId(),
							muxing, path, dashManifest, period, audioAdaptationSet);
				}
			}
		}
    ...
    private Status waitUnilManifesStatusFinished(AbstractApiResponse manifest) throws InterruptedException, BitmovinApiException,
            IOException, RestException, URISyntaxException, UnirestException
    {
			...
			throws InvalidCodecException, BitmovinApiException, UnirestException, IOException, URISyntaxException
	{
		...
		{
		case H264: return createH264VideoConfiguration((H264VideoProfile) videoProfile);
		case H265: return createH265VideoConfiguration((H265VideoProfile) videoProfile);
		case VP9: return createVP9VideoConfiguration((VP9VideoProfile) videoProfile);
		default: throw new InvalidCodecException("AAC is not a video codec");
		}
	}
	...
	private Muxing createMuxing(MuxingType type, String format, Encoding encoding, S3Output output,
			...
			throws BitmovinApiException, IOException, RestException, URISyntaxException, UnirestException
	{
		String path = String.format(format, profile.bitrate, profile.getCodecType().toString().toLowerCase(), type.toString().toLowerCase());
    ...
    private void addDashRepresentationToAdaptationSet(DashMuxingType type, String encodingId, String streamId,
    		...
    		DashSegmentedRepresentation r = (muxing instanceof FMP4Muxing) ? new DashFmp4Representation() : new DashWebmRepresentation();
		r.setType(type);
